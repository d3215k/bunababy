...
<div style="overflow: auto; max-height: 520px;">
    <div
        ...
        <!-- Calendar frame -->
        <div
            style="position: sticky; top: 0; left: 0; z-index: 30; grid-column-start: 1; grid-row-start: 1; font-size: 0.875rem; line-height: 1.25rem; font-weight: 500; background-color: #f8fafc; border-bottom: 1px solid #f1f5f9; background-clip: padding-box; color: #0f172a; height: {{ $headerHeight }}px; display: flex; align-items: center;">
        </div>
        @foreach ($titles as $item)
            <div
                style="grid-row-start: 1; grid-column-start: {{ $loop->iteration + 1 }}; position: sticky; top: 0; z-index: 10; background-color: #f8fafc; border-bottom: 1px solid #f1f5f9; background-clip: padding-box; color: #0f172a; {{ $titleTextStyle }} font-weight: 500; text-align: center; width: {{ $titleWidth }}px; min-width: {{ $titleWidth }}px; max-width: {{ $titleWidth }}px; height: {{ $headerHeight }}px; display: flex; align-items: center; justify-content: center; overflow: hidden;">
                @if ($titleClamp)
        ...
        @foreach ($times as $time)
            <div
                style="position: sticky; top: 0; left: 0; z-index: 20; grid-row-start: {{ $time['row-start'] }}; grid-column-start: 1; border-right: 1px solid #f1f5f9; font-size: 0.75rem; line-height: 1rem; padding: 0.375rem; padding-top: 0; text-align: right; color: #94a3b8; text-transform: uppercase; background-color: #f8fafc; font-weight: 500; height: {{ $rowHeight }}px; width: {{ $timeWidth }}px; min-width: {{ $timeWidth }}px; max-width: {{ $timeWidth }}px; display: flex; align-items: flex-start; justify-content: flex-end;">
                {{ $time['time'] }}
            </div>
