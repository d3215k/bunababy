    ...
    <div class="flex flex-col overflow-hidden bg-white rounded shadow-sm">
        ...
        <div>
            ...
            ">
                ...
                w-fit
                grid gap-0
                grid-cols-[70px,repeat({{ $titles->count() }},210px)]
            ...
            ">
                <!-- Calendar frame -->
                <div
                    class="sticky top-0 left-0 z-20 col-start-1 row-start-1 text-sm font-medium bg-white border-b border-slate-100 bg-clip-padding text-slate-900 h-10 flex items-center">
                </div>
                @foreach ($titles as $item)
                    <div
                        class="row-start-1 col-start-{{ $loop->iteration + 1 }} sticky top-0 z-10 bg-white border-slate-100 bg-clip-padding text-slate-900 border-b text-xs font-medium text-center w-[210px] min-w-[210px] max-w-[210px] flex items-center justify-center overflow-hidden">
                ...
                @foreach ($times as $time)
                    <div
                        class="row-start-{{ $time['row-start'] }} col-start-1 border-slate-100 border-r text-xs p-1.5 pt-0 text-right text-slate-400 uppercase sticky z-20 left-0 bg-white font-medium h-[25px] w-[70px] min-w-[70px] max-w-[70px] flex items-start justify-end">
                        {{ $time['time'] }}</div>

                    @for ($i = 2; $i <= $titles->count() + 1; $i++)
                        <div
                            class="row-start-{{ $time['row-start'] }} col-start-{{ $i }} border-slate-100 border-b border-r h-[25px] w-[210px] min-w-[210px] max-w-[210px]">
                        </div>
                    @endfor
                @endforeach
                ...
                @foreach ($schedules as $schedule)
                    <a target="_blank" href="{{ route('filament.admin.resources.orders.edit', $schedule['id']) }}" wire:key="{{ $schedule['id'] }}"
                        ...
                        <div class="flex flex-col overflow-y-auto h-full w-full text-xs leading-tight">
                            ...
                            <span class="mt-0.5 truncate text-[10px]">{{ $schedule['midwife_name'] }}</span>
                        </div>
                    </a>
                @endforeach
